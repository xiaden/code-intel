...
class TestVitestStep:
    ...
    def test_vitest_failure_with_none_outputs_uses_test_failure(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        import mcp_code_intel.tools.lint_project_frontend as mod
        ...
        with patch(
            ...
            side_effect=[
                _make_proc_text(stdout="", stderr="", returncode=0),
                _make_proc_text(stdout="", stderr="", returncode=0),
                _make_proc_text(stdout=None, stderr=None, returncode=1),
            ],
        ):
            result = lint_project_frontend()
        ...
        assert any(
            error["tool"] == "vitest"
            and error["code"] == "test-failure"
            and "<no output captured>" in error["message"]
            for error in result["errors"]
        )
        assert not any(
            error["tool"] == "vitest" and error["code"] == "tool-error"
    ...
    def test_vitest_failure_with_non_utf8_bytes_is_decoded_safely(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        import mcp_code_intel.tools.lint_project_frontend as mod
