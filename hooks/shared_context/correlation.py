...
def correlate_subagent_start(
    ...
    duplicate_pendings = [
        envelope
        for envelope in pending_matches
        if envelope is not chosen_pending and _created_seq(envelope) >= 0
    ]
    ...
    reserved_item_ids = _normalize_effective_item_ids(
        chosen_pending.get("reserved_next_child_item_ids")
    )
    correlation_id = _normalize_non_empty_string(chosen_pending.get("correlation_id"))
    if (
        parent_agent_id is None
        or effective_item_ids is None
        or reserved_item_ids is None
    ...
    try:
        ...
        return {
            "active_envelope": existing_raced_active
            if existing_raced_active is not None
...
def capture_pretooluse_spawn(
    ...
    if (
        ...
        raise ValueError(
            "session_id, tool_use_id, parent_agent_id, transcript_path, "
            "and cwd must be non-empty strings."
        )
    ...
    normalized_tool_input_summary = _normalize_json_value(
        cast("dict[str, object]", tool_input_summary)
    )
    ...
    try:
        ...
        return {
            "correlation_id": normalized_tool_use_id,
            "pending_envelope": existing_raced_pending
            if existing_raced_pending is not None
            else pending_envelope,
            "status": "duplicate",
        }
