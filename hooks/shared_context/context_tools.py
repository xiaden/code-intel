...
def _parse_context_item(record: dict[str, object]) -> StoredContextItem | None:
    ...
    if (
        item_id is None
        or source_op is None
        or owner_agent_id is None
        or owner_lineage is None
        or kind is None
    ):
        return None
...
def _matches_scope(
    item: StoredContextItem, *, current_agent_id: str, current_lineage: list[str]
) -> bool:
    ...
    *,
    ...
    if active_envelope is not None:
        ...
        for item in state.ordered_items:
            ...
            if not _matches_scope(
                item,
                current_agent_id=normalized_current_agent_id,
                current_lineage=normalized_current_lineage,
            ):
                continue
    ...
    for item in ordered_items:
        ...
        if (
            ...
            and payload_bytes_used + item_payload_bytes > max_payload_bytes
        ):
            break
